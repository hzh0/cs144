...
private:
  ...
  std::unordered_map<uint32_t , std::pair<EthernetAddress , size_t>> ip_ethAddr_time_{};
  uint64_t cur_time_{0};
  std::deque<EthernetFrame> pushed_{};
  std::unordered_map<uint32_t ,std::vector<InternetDatagram>> queued_{};
  std::unordered_map<uint32_t, uint64_t> ip_requesttime_{};
...
public:
  ...
  Address getIp() const { return ip_address_;}
};
